...
WebProvisioningParser* web_provisioning_parser_construct (GType object_type, const char* path) {
	...
	__catch0_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_error ("moonshot-webp-parser.vala:220: Could not retreive file size");
			_g_error_free0 (e);
		}
	}
	__finally0:
...
void web_provisioning_parser_parse (WebProvisioningParser* self) {
	...
	__catch1_g_error:
	{
		...
		_inner_error_ = NULL;
		{
			g_error ("moonshot-webp-parser.vala:237: Could not parse %s, invalid content", self->priv->path);
			_g_error_free0 (e);
		}
	}
	__finally1:
...
gint web_provisioning_main (char** args, int args_length1) {
	...
	if (args_length1 < 2) {
		g_error ("moonshot-webp-parser.vala:246: Usage %s [-a] WEB_PROVISIONING_FILE", args[0]);
	}
	if (!g_file_test (args[1], G_FILE_TEST_EXISTS | G_FILE_TEST_IS_REGULAR)) {
		g_error ("moonshot-webp-parser.vala:251: %s does not exist", args[1]);
	}
